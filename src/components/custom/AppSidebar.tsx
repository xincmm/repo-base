...
export const AppSidebar = async ({ owner, repo }: { owner: string; repo: string }) => {
  ...
  if (resourceId) {
    const grouped = Object.groupBy(
      ...
      ({ metadata }) => (metadata?.owner === owner && metadata?.repo === repo ? "repo" : "other")
    );
    ...
    return (
      <Sidebar>
        ...
        <SidebarContent>
          <SidebarGroup>
            <NewThreadWithRepoButton owner={owner} repo={repo} resourceId={resourceId} />
            <SidebarGroupLabel>
              ...
              {grouped["other"]?.map((t) => (
                <SidebarMenuItem key={t.id} className="truncate">
                  <SidebarMenuButton className="h-min gap-0.5" asChild>
                    <Link href={`/${t.metadata?.owner}/${t.metadata?.repo}/${t.id}`} className="flex-col items-start">
